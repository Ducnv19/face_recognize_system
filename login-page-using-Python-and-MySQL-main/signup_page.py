...
class SignUp:
    ...
    def signup_func(self):
        if self.fname_txt.get()=="" or self.lname_txt.get()=="" or self.email_txt.get()=="" or self.questions.get()=="Select" or self.answer_txt.get()=="" or self.password_txt.get() == "":
            ...
            try:
                connection = mysql.connector.connect(host="localhost",username="root",password="Duc.nv184392",database="face_recognizer")
                cur = connection.cursor()
                cur.execute("select * from signup where email=%s",self.email_txt.get())
                connection.commit()
                connection.close()
                row=cur.fetchone()
